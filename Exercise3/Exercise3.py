...
var = 0.02 # We'll have to experiment with different values of this parameter.
...
_ = run_chain(chain=theta_chain, iterations=burn_in+1, var=var, eps=eps)
...
current_ESS = 0
check_ESS = 5000 # Check the ESS of the current chain every 500 iterations.
max_iter = 100000
accepted = 0
...
while current_ESS < target_ESS:
    ...
    accepted += run_chain(chain=theta_chain, iterations=check_ESS, var=var, eps=eps)
    # Calculate ESS on the current chain
    current_ESS = calculate_ESS(chain=np.array(theta_chain), M=50)
    print(current_ESS)
    if len(theta_chain) > max_iter:
        print("\n[Warning]: Maximum iterations reached without hitting target ESS.")
        break

n_total_iters = len(theta_chain)-1
print(f"Acceptance rate: {100*(accepted / n_total_iters):.2f}%")
